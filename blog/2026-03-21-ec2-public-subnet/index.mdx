...
EC2의 Instance Metadata Service를 v2로 강제한다. `httpTokens: REQUIRED`와 `httpPutResponseHopLimit: 1`을 설정하면 대부분의 SSRF를 통한 metadata 접근을 차단할 수 있다. IMDSv2는 metadata 요청 전에 PUT 요청으로 세션 토큰을 먼저 획득해야 하는데, 일반적인 SSRF 취약점으로는 PUT 요청을 보내기 어렵다. hop limit 1은 네트워크 홉을 거친 요청이 토큰을 받지 못하게 차단하며, Docker bridge network 위의 컨테이너에서도 IMDS에 접근할 수 없게 만든다.
